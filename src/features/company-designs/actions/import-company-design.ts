...
export async function importCompanyDesign(
  designId: string,
): Promise<ImportCompanyDesignResult> {
  if (!designId.trim())
    return { ok: false, error: "Missing design to import." };
  ...
  try {
    ...
    const message =
      error instanceof ApiError
        ? error.message
        : "Failed to import the design. Please try again.";
