...
export async function getLeadForEdit(id: string): Promise<GetLeadForEditResult> {
  try {
    const lead = await apiFetch<LeadRes>(`/api/Leads/Get?id=${encodeURIComponent(id)}`, {
      auth: true
    });
    const contact = lead.leadContacts?.[0]?.contact;
    const contactId = lead.leadContacts?.[0]?.contactId ?? contact?.id ?? "";
    if (!contactId) return { ok: false, error: "This lead has no linked contact to edit." };
    ...
    return {
      ok: false,
      error: error instanceof ApiError ? error.message : "Failed to load the lead."
    };
  }
}
