...
import { addActivityLog } from "../../actions/lead-activity";
import type {
  LeadActivityEntry,
  LeadOverview,
...
export function ActivityTab({
  ...
  function add() {
    startTransition(async () => {
      const res = await addActivityLog(leadId, { summary, details });
      if (res.ok) {
    ...
    <TabLayout overview={overview}>
      <section className="bg-card rounded-2xl border p-5">
        <header className="flex items-center justify-between gap-3 pb-2">
          <h3 className="text-foreground text-lg font-semibold">Activity</h3>
        ...
        {activity.length === 0 ? (
          ...
          <Timeline defaultValue={activity.length} className="mt-4">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Record a manual activity on this lead.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="la-summary">Summary</Label>
              <Input
                id="la-summary"
                value={summary}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="la-details">Details</Label>
              <Textarea
                id="la-details"
                rows={3}
                value={details}
