...
export async function createUser(input: CreateUserInput): Promise<CreateUserResult> {
  ...
  if (!parsed.success) {
    ...
    return { ok: false, fieldErrors: { email: fe.email?.[0], password: fe.password?.[0] } };
  }
  const { email, password, role } = parsed.data;

  const path = role === "Admin" ? "/api/UsersA/CreateAdmin" : "/api/UsersA/CreateDesignAdmin";
  const body: RegisterUserReq = { email, password };

  try {
    const res = await apiFetch<MessageResponseDto>(path, { method: "POST", auth: true, body });
    return { ok: true, message: res.message ?? "User created." };
  } catch (error) {
    return {
      ok: false,
      error: error instanceof ApiError ? error.message : "Failed to create the user."
    };
  }
}
