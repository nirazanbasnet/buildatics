...
export async function getForgotPasswordLink(email: string): Promise<ForgotPasswordLinkResult> {
  const body: ForgotPasswordReq = { email };
  try {
    const res = await apiFetch<ForgotPasswordRes>("/api/UsersA/GetForgotPasswordLink", {
      method: "POST",
      auth: true,
      body
    });
    ...
    return {
      ok: false,
      error: error instanceof ApiError ? error.message : "Failed to generate the reset link."
    };
  }
}
