...
export async function forceAcceptTerms(
  ...
  try {
    ...
    return { ok: true, message: res.message ?? "Terms accepted on behalf of the user." };
  } catch (error) {
    return {
      ok: false,
      error: error instanceof ApiError ? error.message : "Failed to accept the terms."
    };
  }
}
...
export async function resetTermsAcceptance(userId: string): Promise<UserActionResult> {
  try {
    const res = await apiFetch<MessageResponseDto>("/api/UsersA/ResetTermsAndConditionsAcceptance", {
      method: "PATCH",
      auth: true,
      body: { userIds: [userId] }
    });
    ...
    return {
      ok: false,
      error: error instanceof ApiError ? error.message : "Failed to reset terms acceptance."
    };
  }
}
