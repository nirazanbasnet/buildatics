...
import { STATIC_CONTACT_ROLES } from "../../lib/detail-static";
...
type DetailContactsProps = {
  // Real developer name overrides the placeholder; other roles are design placeholders (no API).
  developer: string;
  onEdit?: () => void;
};

export function DetailContacts({ developer, onEdit }: DetailContactsProps) {
  const reduceMotion = useReducedMotion() ?? false;
  const contacts = STATIC_CONTACT_ROLES.map((c) =>
    c.role === "Developer" && developer !== "—"
      ? { ...c, value: developer }
      : c,
  );
