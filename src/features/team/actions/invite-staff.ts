...
export async function inviteStaff(input: InviteStaffInput): Promise<InviteResult> {
  ...
  try {
    ...
    const roleId = data.roleId && data.roleId !== ROLE_NONE ? data.roleId : undefined;
    ...
    if (roleId && staffId) {
      ...
      try {
        ...
        return {
          ok: true,
          message: "Invite sent, but the role could not be assigned. Set it from the member's actions."
        };
      }
    }
    ...
    return {
      ok: false,
      error: error instanceof ApiError ? error.message : "Failed to send the invite."
    };
  }
}
