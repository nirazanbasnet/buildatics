...
import { addQuoteSchema, toUtcIso, type AddQuoteInput } from "../lib/quote-form-schema";
...
export async function createQuote(input: AddQuoteInput): Promise<CreateQuoteResult> {
  const parsed = addQuoteSchema.safeParse(input);
  if (!parsed.success) {
    const fe = parsed.error.flatten().fieldErrors;
    return { ok: false, fieldErrors: { title: fe.title?.[0], leadId: fe.leadId?.[0] } };
  }
  ...
  try {
    ...
    return { ok: false, error: error instanceof ApiError ? error.message : "Failed to create the quote." };
  }
}
