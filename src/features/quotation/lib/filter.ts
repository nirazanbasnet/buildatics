...
export const QUOTES_FILTER_DEFAULTS: QuotationsFilterState = {
  search: "",
  status: "all",
};
...
export function applyQuoteFilters(
  ...
  return rows.filter((row) => {
    if (filters.status !== "all" && String(row.statusValue) !== filters.status)
      return false;
...
export function countActiveQuoteFilters(
  filters: QuotationsFilterState,
): number {
  let n = 0;
