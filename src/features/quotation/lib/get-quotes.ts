...
import type { LeadQuoteLineItemRes, LeadQuoteRes, LeadQuoteResPage } from "./dto";
...
async function getQuotesForLead(leadId: string): Promise<LeadQuoteRes[]> {
  ...
  while (pageNumber <= QUOTES_MAX_PAGES) {
    const res = await apiFetch<LeadQuoteResPage>(
      ...
      { method: "POST", auth: true, body: { pageNumber, pageSize: QUOTES_PAGE } }
    );
...
export async function getAllQuotes(): Promise<QuotationRow[]> {
  ...
  const perLead = await pooled(leads, LEAD_CONCURRENCY, (lead) => getQuotesForLead(lead.id));
...
export function getLineItems(leadId: string, leadQuoteId: string): Promise<LeadQuoteLineItemRes[]> {
...
type LeadContextRes = {
  siteAddress?: { street?: string; suburb?: string; city?: string; areaCode?: string } | null;
  lotNo?: string | null;
...
export async function getQuoteLeadContext(
  ...
  try {
    const lead = await apiFetch<LeadContextRes>(`/api/Leads/Get?id=${encodeURIComponent(leadId)}`, {
      auth: true
    });
