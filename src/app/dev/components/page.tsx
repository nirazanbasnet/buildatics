    ...
    <main className="mx-auto max-w-screen-xl space-y-12 p-8">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          Auto-discovered templates from{" "}
          <code>/templates/{"{feature}"}/Variant*.tsx</code>. Iterate designs
          here before promoting to <code>src/</code>. Dev-only route.
        </p>
      </header>
      ...
      {features.length === 0 ? (
        <div className="text-muted-foreground rounded-md border p-8 text-center text-sm">
          ...
          <code>templates/{"{feature}"}/Variant1.tsx</code> to begin.
        </div>
      ) : (
          ...
          <section key={feature.name} className="space-y-4">
            <div className="flex items-baseline gap-3">
              ...
              <span className="text-muted-foreground text-xs">
                {feature.variants.length} variant
                {feature.variants.length === 1 ? "" : "s"}
              </span>
            </div>
            {feature.notes ? (
